...
void sort(vector<int> &v){
    ...
    while(i<j){ // loop lagalam i jadi j r thaka choto hoy 
        if(v[i]<1 && v[j]>=1){  // amar jadi element 1 r thaka choto hoy tahola negative . ar j positive tahola swap kore dabo 
        ...
        if(v[i]>=1) i++; // check i  positive or not
        if(v[j]<1) j--;// check j negative or not
    }
    cout<<endl;
    return ;
}
