...
//         if(correctIdx==i || arr[i]==n) i++; // jodi corrctIdx element tar nijer position thaka || ar first element jadi n hoy
...
int main(){
    ...
    int actualsum= n*(n+1)/2;  // 1 to n no of sum
    int sum =0;
    for(int i=0;i<n;i++){ // jai array ta doa acha tar sum
        sum+=arr[i];
    }
    cout<<actualsum-sum;
}