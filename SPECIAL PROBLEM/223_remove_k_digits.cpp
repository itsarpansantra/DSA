...
string remove_k(string str,int k){
    ...
    // if any k present then
    while(!st.empty() && k>0){
        k--;
        st.pop();
    }
    // now remaing stack element in string
    string ans="";
    while(!st.empty()){
        ans+=st.top();
        st.pop();
    }
    // reverse it original from
    reverse(ans.begin(),ans.end());
    // now remove 0 in first
    int i=0;
    while(i<ans.size() && ans[i]=='0')i++;
    ans = ans.substr(i); // tatogulo 0 out
    if(ans=="") return '0';
    return ans;
}
int main(){
