...
*/
// basically find out all aunts are droped then max time 
# include<iostream>
# include<vector>
using namespace std;
int lastMoment(int n , vector<int>&left,vector<int>&right){
    ...
    for(int i=0;i<left.size();i++){
        ans = max(ans , left[i]); // left[i]-0 same left[i]
    }

    // for right
    for(int i=0;i<right.size();i++){
        ans = max(ans , n-right[i]); // right towards n -> n-right[i]
    }
    return ans;
}
int main(){
